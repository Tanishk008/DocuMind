...
const tee_1 = require("./lib/tee");
const graphrag_1 = require("./lib/graphrag");
...
function extractJpegsFromPdfBuffer(buffer, outputDir, docNameClean) {
    const imageUrls = [];
    let index = 0;
    let pos = 0;
    try {
        ...
        while (true) {
            ...
            if (buffer[dataStart] === 13)
                dataStart++; // \r
            if (buffer[dataStart] === 10)
                dataStart++; // \n
            ...
            if (jpegData.length > 500) {
                ...
                try {
                    fs_1.default.writeFileSync(filePath, jpegData);
...
async function extractDocxTextAndImages(buffer, outputDir, docNameClean) {
    let imageCounter = 0;
    const imageUrls = [];
    try {
        fs_1.default.mkdirSync(outputDir, { recursive: true });
        const convertImage = mammoth_1.default.images.imgElement((element) => {
            imageCounter++;
        ...
        const result = await mammoth_1.default.convertToHtml({ buffer }, { convertImage });
...
async function askOpenRouter(prompt, model = OPENROUTER_MODELS[0]) {
...
async function withOpenRouterFallback(promptFactory, resultParser) {
...
async function withFastLlm(factory) {
    const keys = getAllGroqKeys();
    if (keys.length === 0)
        throw new Error("NO_GROQ_KEY_CONFIGURED");
...
async function withKeyModelFallback(factory) {
    const keys = getAllGroqKeys();
    if (keys.length === 0)
        throw new Error("NO_GROQ_KEY_CONFIGURED");
    ...
    for (let ki = 0; ki < totalKeys; ki++) {
        ...
        for (let mi = modelStart; mi < totalModels; mi++) {
            ...
            try {
                ...
                if (is413 && mi < totalModels - 1)
                    break;
                if (is429 || isAuth || is413)
                    continue;
                continue;
            }
        }
...
const makeLlm = (apiKey, model) => new groq_1.ChatGroq({
    apiKey,
    model,
    temperature: 0.2,
    maxRetries: 0,
});
async function askSingleQuestion(llm, fullContext, graphContext, question, sources) {
...
async function getUnifiedUser(email, authConn, dataConn) {
...
app.post("/api/auth/signup", async (req, res) => {
    try {
        ...
        const authConn = await (0, mongodb_1.connectToAuthDatabase)();
        const UserModel = authConn.model("User");
        ...
        const hashedPassword = tee_1.TrustedExecutionEnvironment.secureHash(password);
        await UserModel.create({ email, password: hashedPassword, name });
        const dataConn = await (0, mongodb_1.connectToDataDatabase)();
        const safeUser = await getUnifiedUser(email, authConn, dataConn);
        // Audit log: SIGNUP
        try {
            const dataConn2 = await (0, mongodb_1.connectToDataDatabase)();
...
app.put("/api/profile", async (req, res) => {
    try {
        ...
        const dataConn = await (0, mongodb_1.connectToDataDatabase)();
        const UserModel = authConn.model("User");
        const updatedUser = await UserModel.findOneAndUpdate({ email }, { $set: updateFields }, { new: true });
        if (!updatedUser) {
...
app.post("/api/user/documents", async (req, res) => {
    try {
        ...
        const existingDocsMap = new Map(existingDocs.map((doc) => [doc.name, doc]));
        ...
        const cleanDocs = documents.map((d) => {
            const existingDoc = existingDocsMap.get(d.name);
...
app.post("/api/user/questions", async (req, res) => {
    try {
        ...
        await UserModel.findOneAndUpdate({ email }, { $set: { totalQueries: cleanQuestions.length } });
        return res.json({
            success: true,
...
app.post("/api/auth/otp", async (req, res) => {
    try {
        ...
        const otp = tee_1.TrustedExecutionEnvironment.generateSecureOtp();
...
app.put("/api/auth/reset-password", async (req, res) => {
    try {
        ...
        const hashedPassword = tee_1.TrustedExecutionEnvironment.secureHash(newPassword);
        // Update password
...
app.get("/api/user/documents/:docId/content", async (req, res) => {
    try {
        ...
        if (!doc.content)
            return res.status(404).json({ error: "No content stored for this document. Please re-upload." });
...
app.put("/api/user/documents/:docId/tags", async (req, res) => {
    try {
        ...
        const dataConn = await (0, mongodb_1.connectToDataDatabase)();
        const DocModel = dataConn.model("UserDocument");
        await DocModel.findOneAndUpdate({ id: docId, userEmail: email.toLowerCase() }, { $set: { tags } });
        return res.json({ success: true });
    }
    catch (error) {
...
app.post("/api/summarize", upload.single("file"), async (req, res) => {
    try {
        ...
        const combinedResult = await withFastLlm(async (apiKey, model) => {
            ...
            const prompt = prompts_1.ChatPromptTemplate.fromTemplate(`You are DocuMind AI. Do both tasks below based on the document text.
...
JSON:`);
        ...
        if (email && docId) {
            try {
                const dataConn = await (0, mongodb_1.connectToDataDatabase)();
                const DocModel = dataConn.model("UserDocument");
                await DocModel.findOneAndUpdate({ id: docId, userEmail: email.toLowerCase() }, { $set: { summary: summaryResult, documentType: docTypeResult } });
            }
            catch (e) { }
        }
    ...
    try {
        ...
        return res.json({
            ...
            stats: {
                ...
                topUsers: topUsers.map((u) => ({ email: u.email, name: u.name, queries: u.totalQueries })),
                docTypeDistribution: docTypeDistribution.map((d) => ({ type: d._id || "General", count: d.count }))
            }
        });
    }
    catch (error) {
...
app.post("/api/analyze", upload.array("files"), async (req, res) => {
    try {
        ...
        const allDocs = [];
        const fileGraphs = [];
        const textSplitter = new text_splitter_1.RecursiveCharacterTextSplitter({ chunkSize: 100000, chunkOverlap: 2000 });
        for (const file of files) {
            ...
            if (dbConnected && docModel) {
                ...
                if (dbDoc && dbDoc.nodes && dbDoc.nodes.length > 0) {
                    ...
                    fileGraph = {
                        nodes: dbDoc.nodes.map((n) => ({ id: n.id, label: n.label, type: n.type, description: n.description })),
                        edges: dbDoc.edges.map((e) => ({ source: e.source, target: e.target, relation: e.relation, description: e.description })),
                    };
                    foundInDb = true;
                    // Backfill missing content bytes if knowledge graph already exists
                    if (!dbDoc.content) {
                        try {
                            await docModel.updateOne({ _id: dbDoc._id }, { $set: { content: file.base64Content } });
            ...
            if (!foundInDb) {
                ...
                const graphSplitter = new text_splitter_1.RecursiveCharacterTextSplitter({ chunkSize: 3000, chunkOverlap: 200 });
                ...
                for (let i = 0; i < graphChunks.length; i += BATCH_SIZE) {
                    ...
                    const promises = batch.map((chunk) => withFastLlm(async (apiKey, model) => {
                        const llm = makeLlm(apiKey, model);
                        return (0, graphrag_1.extractGraphFromChunk)(chunk.pageContent, llm);
                    }));
                ...
                fileGraph = (0, graphrag_1.consolidateGraphs)(chunkGraphs);
                ...
                if (dbConnected && docModel) {
                    try {
                        ...
                        await docModel.findOneAndUpdate({ userEmail: email.toLowerCase(), name: file.name }, {
                            $set: {
        ...
        const sanitizedDocs = allDocs.map((doc) => new documents_1.Document({ pageContent: String(doc.pageContent), metadata: doc.metadata }));
        ...
        const masterGraph = (0, graphrag_1.consolidateGraphs)(fileGraphs);
        // --- Batch prompt template (with citation extraction) ---
        const batchPrompt = prompts_1.ChatPromptTemplate.fromTemplate(`
            ...
            try {
                ...
                try {
                    seeds = await withFastLlm(async (apiKey, model) => {
                        const llm = makeLlm(apiKey, model);
                        return (0, graphrag_1.extractSeedEntities)(q, llm);
                    });
                }
                catch {
                    seeds = [];
                }
                const subgraph = (0, graphrag_1.retrieveSubGraph)(masterGraph.nodes, masterGraph.edges, seeds, 2);
                const graphContext = (0, graphrag_1.formatGraphAsText)(subgraph);
                let singleResult;
                try {
